...
async def create_event(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(text='Введите тип мероприятия: Wedding, Birthday, Corporate, '
...
async def add_event_type(message: Message, state: FSMContext) -> None:
    walid_types = ['wedding', 'birthday', 'corporate', 'свадьба', 'день рождения', 'корпоратив']
    if message.text.lower() not in walid_types:
        await message.answer(text='Ваш вариант не подходит ни под один из предложенных. Пожалуйста выберите '
                                  'тип из предложенных: Wedding, Birthday, Corporate, '
                                  'Свадьба, День рождения, Корпоратив')
        return
    ...
    await message.answer(text=f'Отлично! Мы начали создание мероприятия "{html.escape(message.text)}"! Едем дальше,'
                              f'напиши дату мероприятия в формате ГГГГ-ММ-ДД')
...
async def add_event_date(message: Message, state: FSMContext):
    ...
    await message.answer(text='Я записал!, Введие количесво гостей в формате (пример: 32)')
...
async def add_num_guests(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        await state.set_state(Form.event_info)
        await message.answer(text='Отлично, двигаемся дальше! Теперь вам нужно рассказать все что должно входить в '
                                  'состав мероприятия, а именно: Формат: что вы именно хотите увидеть на мероприятии\n'
                                  'Имена гостей: желательно ввести гостей по значимости, от самого значимого (мама/папа) '
                                  'к менее значимым\n'
                                  'Мелочи: те самые мелочи которые вы хотели бы видеть\n'
                                  'Не переживайте, после заполнения заявки я с вами свяжусь и мы уже вместе пройдемся '
                                  'по заявке :)')
    else:
...
async def add_contact_phone(message: Message, state: FSMContext):
    if re.search(r'^\d{1}-\d{3}-\d{3}-\d{2}-\d{2}$', message.text):
        ...
        event_text = (f"Мероприятие создано!\n"
                      f"Тип: {data['type_event']}\n"
                      f"Дата: {data['event_date']}\n"
                      f"Гостей: {data['num_guests']}\n"
                      f"Информация: {data['event_info']}\n"
                      f"Телефон: {data['contact_phone']}")
        id_admin = 498037596
        if id_admin == message.from_user.id:
            await message.answer(text=event_text, reply_markup=keyboards.main_admin_kb)
        else:
            await message.answer(text=event_text, reply_markup=keyboards.main_user_kb)
...
async def info_key_for_user(callback: CallbackQuery):
    ...
    responses = (f'Тип мероприятия: {event_info['type']}\n'
                 f'Дата проведения: {event_info['date']}\n'
                 f'Количество гостей: {event_info['num_guests']}\n'
                 f'Информация о мероприятии: {event_info['event_info']}\n'
                 f'Номер телефона: {event_info['telephone']}\n\n'
    )


    await callback.message.answer(text=f'Ваше мероприятие: \n\n{responses}')
